...
## Status
Proposed. Implementation: **PROPOSED** (the documentation architecture layer
itself — it is unratified and untracked, so it is not yet ESTABLISHED) /
**TARGET** (machine validation of the spec field). The layer becomes an
established, authoritative baseline only once a human completes the ratification
checklist in [`decisions/README.md`](README.md#ratification-checklist).
...
5. **Ratify via one coherent change.** Adopting this layer is a single
   ...
   ADR's `Status`, and the specs-README boundary, committed together. The exact
   steps are the [ratification checklist](README.md#ratification-checklist).
   This ADR pass does **not** ratify anything; the boundary stays `null`.
...
## Open questions
- **Where should the machine validator live once ratified?** The natural home is
  alongside the existing spec-metadata scanner used by the release tooling,
  extended to check `architecture_version` for specs past the adoption boundary,
  with focused tests. This is a small, contained follow-up — intentionally **not**
  implemented in this pass.
- **What is the exact adoption boundary number?** It is the highest spec number
  existing at ratification; it stays `null` until a human ratifies.
- **Should ratification itself be recorded as a spec?** Likely yes, so the
  boundary and the ratification act are auditable in the same convention as
  every other change.
