Prompt #1 — Implement Spec Task 9003-dogfood-scenario-c

Implement exactly what docs/sdd/9003-dogfood-scenario-c/spec.md requires. Do not add, remove, or
reinterpret requirements. If the spec is unclear or contradictory,
stop and report a blocking issue instead of guessing.

Write these files in .ai-runs/tasks/9003-dogfood-scenario-c/ (the task's runtime evidence folder,
NOT the spec's own directory):
  - .ai-runs/tasks/9003-dogfood-scenario-c/03-executor-log.md
  - .ai-runs/tasks/9003-dogfood-scenario-c/07-tests.txt
  - .ai-runs/tasks/9003-dogfood-scenario-c/08-executor-summary.md

=== Spec (docs/sdd/9003-dogfood-scenario-c/spec.md) ===
# Dogfood Scenario C — interruption then safe resume

Append a line to INTERRUPT-FIXTURE.txt at the repository root. This is a
deliberately trivial, bounded, safe fixture spec used only to dogfood
SpecRelay's interruption/resume safety (SDD 0085 dogfood scenario C) inside
an isolated harness — it is not real product work.
