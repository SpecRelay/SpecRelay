...
# they never depend on, or mutate, this developer's real .ai/ or .specrelay-runs/
...
# .specrelay-runs/ policy rather than a bare-git-repo artifact). Individual
# tests still commit their own fixture files as needed.
specrelay_test::mktemp_specrelay_project() {
  local dir
  dir="$(specrelay_test::mktemp_project)"
