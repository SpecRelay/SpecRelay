...
# fixture directories — never the real repository's .ai/ or .specrelay-runs/.
...
specrelay_test::assert_contains "workflow inspect reports no workflow found" \
  "$out_empty" "No legacy AI workflow detected"
...
mkdir -p "$proj/.specrelay-runs/tasks"
...
specrelay_test::assert_contains "workflow inspect reports the task run root as existing" \
  "$out" "$proj/.specrelay-runs/tasks (exists)"
...
# --- 11. no CLI command mutates .ai/, .specrelay-runs/, or any task state ----------
...
snapshot() {
  (cd "$proj" && find .ai .specrelay-runs .claude .mcp.json -type f -exec sh -c '
      for f; do
...
specrelay_test::assert_eq "no CLI command mutates .ai/, .specrelay-runs/, .claude/, or .mcp.json" \
  "$before" "$after"
