...
|---|---|---|---|---|
| `start-spec-task.sh` command surface | Runs the legacy engine directly (`run-ai-loop.sh` → `run-workflow.sh` → `run-executor.sh`/`run-reviewer.sh`) | Compatibility shim delegates to `specrelay run <spec>`, translating `--allow-dirty`→`--allow-dirty-baseline`, preserving `--task-id`, exit code, and spec paths with spaces | previously `compat_shim_test.sh` (18 assertions, now removed as unsupported in this standalone repo); real dogfood scenarios A/B (`docs/dogfood-report.md`) ran through this exact code path | PARITY (same lifecycle semantics; superficial banner text differs, per spec section 27) |
| `show-task.sh` command surface | Reads `.ai-runs/tasks/<exact-id>/*` directly, dumps full file contents | Compatibility shim delegates to `specrelay show <task-ref>` (also accepts numeric prefix / partial slug — legacy required the exact id) | previously `compat_shim_test.sh` ("numeric-prefix task ref", "never mutates"), now removed as unsupported in this standalone repo | IMPROVED (task lookup), PARTIAL (output is a compact summary, not a full multi-file dump — documented difference, not silent) |
...
| Direct-vs-shim parity | N/A | For the same fixture spec, `.ai/scripts/start-spec-task.sh` and `tools/specrelay/bin/specrelay run` produce the identical state machine outcome and evidence shape (only cosmetic banner text differs) | previously `compat_shim_test.sh` (fixtures 1 and 5 compared), now removed as unsupported in this standalone repo | PARITY |
